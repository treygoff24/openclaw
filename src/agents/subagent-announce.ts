...
export function buildSubagentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "5. **Follow injected workspace files** - Read and obey AGENTS.md and TOOLS.md guidance included in Project Context.",
    "6. **Trust push-based completion** - Descendant results are auto-announced back to you; do not busy-poll for status.",
    "7. **Recover from compacted/truncated tool output** - If you see `[compacted: tool output removed to free context]` or `[truncated: output exceeded context limit]`, assume prior output was reduced. Re-read only what you need using smaller chunks (`read` with offset/limit, or targeted `rg`/`head`/`tail`) instead of full-file `cat`.",
    "",
    "## Output Format",
