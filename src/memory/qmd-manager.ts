...
export class QmdMemoryManager implements MemorySearchManager {
  ...
  private readonly retainEmbeddings: boolean;
  ...
  private constructor(params: {
    ...
    this.retainEmbeddings = this.resolveRetainEmbeddingsSetting();
  ...
  private async exportSessions(): Promise<void> {
    ...
    if (retainedOrphans > 0) {
      log.debug("qmd sessions export: retaining orphaned markdown files (retainEmbeddings=true)", {
        retained: retainedOrphans,
      });
    }
  }

  private resolveRetainEmbeddingsSetting(): boolean {
